...
class StandaloneServer {
  ...
  _onSessionError(side, sess, err, req, res) {
    ...
    if(res.headersSent) {
      // We need to defer the finish event to the next tick so that it allows
      // the error event to set the error field and deactivate the session before
      // finish event tries to overwrite the state
      res.end('');
    }
