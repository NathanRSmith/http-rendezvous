...
class StandaloneServer {
  ...
  handleRequest(req, res) {
    ...
    // PUT /stream/{id}
      ...
      if(!sess || sess.deleted) return this._replyError(res, HTTP_STATUS.NotFound, SERVER_ERROR.SESS_NOT_FOUND());
    ...
    // GET /stream/{id}/status
      ...
      res.setHeader('content-type', 'application/json');
      if(sess.error) {
        res.end(JSON.stringify({error: sess.error}))
      } else {
        res.end(JSON.stringify({bytes_transferred: sess.bytes_transferred}));
      }
    }
    // GET /stream/{id}
